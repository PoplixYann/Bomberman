...
--Set bombs pos x
function class.setBombsX(dir, value, dt)
  for bombId, bomb in ipairs(bombList) do
    if (dir == "left") then
      bomb.x = bomb.x - value * dt
...
--Create bomb
function class.create(cell, bombSize, detonate)
  local bomb = {}
  local frame, framerate, x, y, w, h

  --Animation
  if (detonate) then
    frame, framerate, x, y, w, h = 3, 3, 0, 0, 50, 50
  else
